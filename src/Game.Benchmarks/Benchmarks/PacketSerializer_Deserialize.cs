...
[MemoryDiagnoser]
public class PacketSerializer_Deserialize
{
    private readonly byte[] _bytes;

    [Params(1, 10, 100, 1000)] public int Iterations { get; set; }

    public PacketSerializer_Deserialize()
    {
        _bytes = new byte[13];
    }
    ...
    public void Class()
    {
        ...
        {
            var obj = new GCHandshake_Class();
            obj.Deserialize(_bytes);
        }
    }
    ...
    public void Struct()
    {
        for (int i = 0; i < Iterations; i++)
        {
            var obj = new GCHandshake_Struct();
            obj.Deserialize(_bytes);
        }
    }
    ...
    public void ReadonlyStruct()
    {
        for (int i = 0; i < Iterations; i++)
        {
            var obj = GCHandshake_ReadonlyStruct.Deserialize(_bytes);
        }
    }
    ...
    public void ReadonlyRefStruct()
    {
        for (int i = 0; i < Iterations; i++)
        {
            var obj = GCHandshake_ReadonlyRefStruct.Deserialize(_bytes);
        }
    }
    ...
    public readonly ref struct GCHandshake_ReadonlyRefStruct(uint handshake, uint time, uint delta)
    {
        ...
        public static GCHandshake_ReadonlyRefStruct Deserialize(ReadOnlySpan<byte> bytes, in int offset = 0)
        {
            var __Handshake =
    ...
    [StructLayout(LayoutKind.Sequential)]
    public class GCHandshake_Class
    {
        public uint Handshake;
        public uint Time;
        public uint Delta;

        public GCHandshake_Class()
        {
        }

        public GCHandshake_Class(uint handshake, uint time, uint delta)
        {
            Handshake = handshake;
            Time = time;
            Delta = delta;
