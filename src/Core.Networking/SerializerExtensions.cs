...
public static class SerializerExtensions
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    public static T ReadValueFromStream<T>(this Stream stream, byte[] buffer)
        where T : ISpanParsable<T> // should match all relevant types
    {
        var length = typeof(T) switch
        {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static string ReadStringFromStream(this Stream stream, byte[] buffer, int size)
    {
    ...
    public static void WriteString(this byte[] bytes, string? str, in int index, in int length)
    {
        ...
        {
            Encoding.ASCII.GetBytes(str, bytes.AsSpan(index, Math.Min(str.Length, length)));
